...
class DMDDLCompiler(compiler.DDLCompiler):
    ...
    def get_column_specification(self, column, **kwargs):
        ...
        if isinstance(column.type, DOUBLE):
            colspec = self.preparer.format_column(column) + " DOUBLE"
...
class DMExecutionContext(default.DefaultExecutionContext):
    ...
    def _setup_ins_pk_from_implicit_returning(self, result, rows):
        self.dialect.trace_process('DMExecutionContext', '_setup_ins_pk_from_implicit_returning', result, rows)
    ...
    def _setup_ins_pk_from_lastrowid(self):
        ...
        if lastrowid is not None:
            ...
            if identity_flag:
                primary_columns = table.primary_key.columns_autoinc_first
                ...
                if need_get_primary_flag:
                    ...
                    if len(result) != 1:
                        ...
                        _autoincrement_column = self.compiled.compile_state.statement.table.primary_key._autoincrement_column
                        self.compiled.compile_state.statement.table.primary_key._autoincrement_column = None
                        self.inserted_primary_key = [
                            self.compiled._inserted_primary_key_from_lastrowid_getter(result, temp_params)]
                        self.compiled.compile_state.statement.table.primary_key._autoincrement_column = _autoincrement_column
